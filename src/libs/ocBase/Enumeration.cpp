...
{
    return isEmpty() ? Enumeration() : Enumeration(name_map, name_map->first());
}

Enumeration Enumeration::last(void) const
{
    return isEmpty() ? Enumeration() : Enumeration(name_map, name_map->last());
}

QQString Enumeration::nameOf(const int value) const
{
    QString result = name_map ? name_map->key(value).toQString() : QString();
    return result;
}
...
void Enumeration::set(const int value)
{
    ...
    {
        value_i = value;
        name_s = name_map->key(value_i).toQString();
    }
    else
        name_map = 0;
...
QQStringList Enumeration::nameFlags(int f)
{
    ...
    if (name_map)
    {
        ...
        foreach (int i, intList)
            if (i && i == (i & f))
                nameList << name_map->key(i).toQString();
    }
    return nameList;
}
