...
public:
    ...
    Q_ENUM(Type);
    KeySeg componentKey(const Type value) const { return enumName(value); }
    Type componentValue(const KeySeg & key) const { return enumValue<Type>(key.toQString().toLocal8Bit().data()); }
